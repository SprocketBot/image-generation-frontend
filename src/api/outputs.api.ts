...
export async function getOutputs(imageTypeId, filename): Promise<string[]> {
  if (!browser) return []
  return await fetch(`api/outputs/${imageTypeId}/${filename}`).then(r=>r.json());
}


export async function downloadOutputImage(reportType: string, projectName, filename:string): Promise<Blob> {
  if (!browser) return;
